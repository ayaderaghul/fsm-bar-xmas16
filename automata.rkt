...
(define (lows)
  ...
  (define body (hash 0 (state 'L (hash 'L 0 'M 0 'H 0))))
  (automaton head body))

(define (mediums)
  (define head (hash 'INITIAL 0 'CURRENT 0 'PAYOFF 0))
  (define body (hash 0 (state 'M (hash 'L 0 'M 0 'H 0))))
  (automaton head body))

(define (highs)
  (define head (hash 'INITIAL 0 'CURRENT 0 'PAYOFF 0))
  (define body (hash 0 (state 'H (hash 'L 0 'M 0 'H 0))))
  (automaton head body))

(define (accommodator)
  (define head (hash 'INITIAL 1 'CURRENT 1 'PAYOFF 0))
  (define body (hash 0 (state 'L (hash 'L 2 'M 1 'H 0))
                     1 (state 'M (hash 'L 2 'M 1 'H 0))
...
(define PAYOFF-TABLE
  (list
   (list (cons LOW LOW)    (cons LOW MEDIUM)    (cons LOW HIGH))
   (list (cons MEDIUM LOW) (cons MEDIUM MEDIUM) (cons 0 0))
   (list (cons HIGH LOW)   (cons 0 0)           (cons 0 0))))
